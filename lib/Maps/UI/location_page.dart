...
class LocationPage extends StatelessWidget {
  ...
  LocationPage(this.lat, this.lng, this.isLoc);

  @override
  Widget build(BuildContext context) {
    return SetLocation(lat, lng, isLoc);
  }
}
...
class SetLocation extends StatefulWidget {
  ...
  SetLocation(this.lat, this.lng, this.isLoc);

  @override
  SetLocationState createState() => SetLocationState(lat, lng, isLoc);
}
...
class SetLocationState extends State<SetLocation> {
  ...
  SetLocationState(this.lat, this.lng, this.isLoc) {
            ...
            if (value[i].locality != null && value[i].locality.length > 1) {
              if (!pageDistroy) {
                setState(() {
                  ...
                  currentAddressPin = value[i].postalCode;
                  enteredSearch = false;
                });
              }
              break;
          ...
          ? Column(
              ...
              children: <Widget>[
                ...
                BottomBar(
                    ...
                    onTap: () {
                      if (isLoc)
                        checkLocation(lat, lng, currentAddressPin);
                      else
                        Navigator.pop(
                            context, BackLatLng(lat, lng, currentAddress));
                    }),
              ],
            )
          : Center(
  ...
  Future<void> checkLocation(lat, lng, addressPin) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    var vendorId = prefs.getString('vendor_id');
    print(addressPin.toString() +
        'vendor====:: ' +
        vendorId.toString() +
        'lat==' +
        lat.toString() +
        'lng' +
        lng.toString());
    var url = addressCheck;
    http.post(url, body: {
      'lat': lat.toString(),
      'lng': lng.toString(),
      'vendor_id': vendorId.toString(),
      'pin_code': addressPin.toString()
    }).then((value) {
